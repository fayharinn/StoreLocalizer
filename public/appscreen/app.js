...
const appscreenBridge = window.AppscreenBridge || {};
appscreenBridge._queue = appscreenBridge._queue || [];
appscreenBridge._ready = appscreenBridge._ready || false;
...
function runWhenBridgeReady(fn) {
    ...
    appscreenBridge._queue.push(fn);
}
...
let ctxRight = null;
let canvasFarLeft = null;
let ctxFarLeft = null;
let canvasFarRight = null;
let ctxFarRight = null;
let sidePreviewLeft = null;
let sidePreviewRight = null;
let sidePreviewFarLeft = null;
let sidePreviewFarRight = null;
...
let noScreenshot = null;

function bindDomElements() {
    canvas = document.getElementById('preview-canvas');
    ctx = canvas ? canvas.getContext('2d') : null;
    canvasLeft = document.getElementById('preview-canvas-left');
    ctxLeft = canvasLeft ? canvasLeft.getContext('2d') : null;
    canvasRight = document.getElementById('preview-canvas-right');
    ctxRight = canvasRight ? canvasRight.getContext('2d') : null;
    canvasFarLeft = document.getElementById('preview-canvas-far-left');
    ctxFarLeft = canvasFarLeft ? canvasFarLeft.getContext('2d') : null;
    canvasFarRight = document.getElementById('preview-canvas-far-right');
    ctxFarRight = canvasFarRight ? canvasFarRight.getContext('2d') : null;
    sidePreviewLeft = document.getElementById('side-preview-left');
    sidePreviewRight = document.getElementById('side-preview-right');
    sidePreviewFarLeft = document.getElementById('side-preview-far-left');
    sidePreviewFarRight = document.getElementById('side-preview-far-right');
...
function syncUIWithState() {
    ...
    const highlightEnabled = !!txt.highlightEnabled;
    document.getElementById('highlight-toggle').classList.toggle('active', highlightEnabled);
    document.getElementById('highlight-options').style.display = highlightEnabled ? 'block' : 'none';
    document.getElementById('highlight-words').value = txt.highlightWords || '';
    document.getElementById('highlight-color').value = txt.highlightColor || '#ffd166';
    ...
    document.querySelectorAll('#text-align button').forEach(btn => {
        btn.classList.toggle('active', btn.dataset.align === (txt.textAlign || 'center'));
    });
    ...
    document.getElementById('text-offset-x').value = txt.offsetX || 0;
    document.getElementById('text-offset-x-value').textContent = formatValue(txt.offsetX || 0) + '%';
...
function setupEventListeners() {
    ...
    document.getElementById('highlight-toggle').addEventListener('click', function() {
        this.classList.toggle('active');
        const enabled = this.classList.contains('active');
        setTextValue('highlightEnabled', enabled);
        document.getElementById('highlight-options').style.display = enabled ? 'block' : 'none';
        updateCanvas();
    });

    document.getElementById('highlight-words').addEventListener('input', (e) => {
        setTextValue('highlightWords', e.target.value);
        updateCanvas();
    });

    document.getElementById('highlight-color').addEventListener('input', (e) => {
        setTextValue('highlightColor', e.target.value);
        updateCanvas();
    });
    ...
    document.querySelectorAll('#text-align button').forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            btn.classList.add('active');
            setTextValue('textAlign', btn.dataset.align);
            updateCanvas();
        });
    });
...
function applyLocalizationData(payload = {}) {
    const languages = Array.isArray(payload.languages) ? payload.languages.filter(Boolean) : [];
    ...
    const syncTextBlock = (text) => {
        if (!text.headlines) text.headlines = { en: '' };
        if (!text.subheadlines) text.subheadlines = { en: '' };
        text.headlineLanguages = [...state.projectLanguages];
        text.subheadlineLanguages = [...state.projectLanguages];

        Object.keys(text.headlines).forEach(lang => {
            if (!state.projectLanguages.includes(lang)) delete text.headlines[lang];
        });
        Object.keys(text.subheadlines).forEach(lang => {
            if (!state.projectLanguages.includes(lang)) delete text.subheadlines[lang];
        });

        state.projectLanguages.forEach(lang => {
            if (!(lang in text.headlines)) text.headlines[lang] = '';
            if (!(lang in text.subheadlines)) text.subheadlines[lang] = '';

            if (headlinesByLang[lang] !== undefined) {
                text.headlines[lang] = headlinesByLang[lang];
            }
            if (subheadlinesByLang[lang] !== undefined) {
                text.subheadlines[lang] = subheadlinesByLang[lang];
            }
        });

        text.currentHeadlineLang = state.currentLanguage;
        text.currentSubheadlineLang = state.currentLanguage;
        text.headline = text.headlines[state.currentLanguage] || '';
        text.subheadline = text.subheadlines[state.currentLanguage] || '';
    };

    syncTextBlock(state.defaults.text);

    if (applyToAll) {
        state.screenshots.forEach(screenshot => {
            syncTextBlock(screenshot.text);
        });
    } else if (state.screenshots[state.selectedIndex]) {
        syncTextBlock(state.screenshots[state.selectedIndex].text);
    }
    ...
    updateLanguageMenu();
    updateLanguageButton();
    switchGlobalLanguage(state.currentLanguage);
...
function updateTextUI(text) {
    ...
    const highlightEnabled = !!text.highlightEnabled;
    const highlightWords = text.highlightWords || '';
    const highlightColor = text.highlightColor || '#ffd166';
    document.getElementById('highlight-toggle').classList.toggle('active', highlightEnabled);
    document.getElementById('highlight-options').style.display = highlightEnabled ? 'block' : 'none';
    document.getElementById('highlight-words').value = highlightWords;
...
function buildHighlightRegex(textSettings) {
...
function getHighlightSegments(line, regex) {
...
function drawLineWithHighlights(context, line, xCenter, y, baseColor, highlightColor, regex) {
    if (!regex) {
...
function drawText() {
    ...
    let currentY = textY;
    const highlightRegex = buildHighlightRegex(text);

    // Draw headline
    if (headline) {
        const fontStyle = text.headlineItalic ? 'italic' : 'normal';
        const baseFont = `${fontStyle} ${text.headlineWeight} ${text.headlineSize}px ${text.headlineFont}`;
        const boldFont = `${fontStyle} ${getBoldWeight(text.headlineWeight)} ${text.headlineSize}px ${text.headlineFont}`;
        ctx.font = baseFont;
        ctx.fillStyle = text.headlineColor;
        const highlightColor = text.highlightColor || '#ffd166';

        const lines = wrapTextWithBreaks(ctx, headline, dims.width - padding * 2);
        ...
        lines.forEach((line, i) => {
            const y = currentY + i * lineHeight;
            lastLineY = y;
            const textWidth = measureStyledLineWidth(ctx, line, baseFont, boldFont);
            const lineX = getAlignedX(align, textWidth, dims, padding, offsetX);
            drawStyledLine(ctx, line, lineX, y, baseFont, boldFont, text.headlineColor, highlightColor, highlightRegex);
    ...
    if (subheadline) {
        ...
        const subBaseFont = `${subFontStyle} ${subWeight} ${text.subheadlineSize}px ${text.subheadlineFont || text.headlineFont}`;
        const subBoldFont = `${subFontStyle} ${getBoldWeight(subWeight)} ${text.subheadlineSize}px ${text.subheadlineFont || text.headlineFont}`;
        ctx.font = subBaseFont;
        const baseColor = hexToRgba(text.subheadlineColor, text.subheadlineOpacity / 100);
        const highlightColor = hexToRgba(text.highlightColor || '#ffd166', text.subheadlineOpacity / 100);
        ctx.fillStyle = baseColor;

        const lines = wrapTextWithBreaks(ctx, subheadline, dims.width - padding * 2);
        ...
        lines.forEach((line, i) => {
            const y = subY + i * subLineHeight;
            const textWidth = measureStyledLineWidth(ctx, line, subBaseFont, subBoldFont);
            const lineX = getAlignedX(align, textWidth, dims, padding, offsetX);
            drawStyledLine(ctx, line, lineX, y, subBaseFont, subBoldFont, baseColor, highlightColor, highlightRegex);
...
function buildStyledHighlightSegments(text, regex) {
...
function measureStyledLineWidth(context, line, baseFont, boldFont) {
...
function getAlignedX(align, textWidth, dims, padding, offsetX) {
...
function drawStyledLine(context, line, xStart, y, baseFont, boldFont, baseColor, highlightColor, regex) {
...
function wrapTextWithBreaks(ctx, text, maxWidth) {
...
// Bridge for hosting environments
appscreenBridge.init = () => {
    if (appscreenBridge._initialized) return;
    appscreenBridge._initialized = true;
    initSync();
    appscreenBridge._ready = true;
    const queue = appscreenBridge._queue.splice(0);
    queue.forEach(fn => fn());
};

appscreenBridge.syncLocalizationData = (payload) => {
    runWhenBridgeReady(() => applyLocalizationData(payload));
};

appscreenBridge.syncAiConfig = (payload) => {
    runWhenBridgeReady(() => applyAiConfig(payload));
};

appscreenBridge.mount = () => {
    if (!appscreenBridge._initialized) {
        appscreenBridge.init();
        return;
    }
    bindDomElements();
